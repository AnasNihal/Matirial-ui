...
async def run_test():
    ...
    try:
        ...
        # -> Input the email address for the user account with active automations and click Continue.
        frame = context.pages[-1]
        # Input the email address for the user account with active automations
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Automation Metrics Updated Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Dashboard overview did not show up-to-date engagement metrics, automation statuses, and trends as expected.')
