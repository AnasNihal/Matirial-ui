...
async def run_test():
    ...
    try:
        ...
        # -> Click on Login to access dashboard for Instagram account connection.
        frame = context.pages[-1]
        # Click on Login to access user dashboard for Instagram account connection
        ...
        # -> Try to sign up or find alternative login method to access dashboard for Instagram connection.
        frame = context.pages[-1]
        # Click Sign up link to create a new account for login
        elem = frame.locator('xpath=html/body/div/div/div/div[2]/div/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input valid email and password, then click Continue to create account and access dashboard.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Instagram OAuth connection successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Instagram OAuth flow did not complete successfully, tokens and profile data were not properly stored and synced as per the test plan.')
